<!--
  ...
  *
  * @Author:    lidoudou
  * @Date:      2023-04-13 15:45:14
  * @Copyright  1024创新实验室
-->
<template>
  <a-modal :open="visible" title="选择易耗品" ok-text="确认" cancel-text="取消" @ok="onSubmit"
...
async function queryData () {
  ...
  try {
    ...
    let queryResult = await consumablesStockApi.queryPage(params);
...
function onSelectChange (selectedRowKeys, selectedRows) {
  // 过滤选中的数据
  let currentConsumablesIdList = tableData.value.map(e => e.consumablesId);

  // 组合数据
  let otherPageSelectedList = selectedRowList.value.filter(e => !currentConsumablesIdList.includes(e.consumablesId));
  selectedRowList.value = otherPageSelectedList.concat(selectedRows);
  selectedRowKeyList.value = selectedRowList.value.map(e => e.consumablesId);
}

// ---------------------------- 弹窗操作 ----------------------------
const visible = ref(false);

function showModal (assetList,customQueryForm) {
  visible.value = true;
  Object.assign(queryForm, queryFormState);
  Object.assign(queryForm, customQueryForm);
  selectedRowKeyList.value = assetList.map(e => e.consumablesId);

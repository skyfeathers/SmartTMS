...
<template>
  <a-select
    ...
    @change="handleChange"
  >
    <a-select-option :value="FLAG_TEXT_ENUM.TRUE.value">
      {{ trueDesc }}
    </a-select-option>
    <a-select-option :value="FLAG_TEXT_ENUM.FALSE.value">
      {{ falseDesc }}
    </a-select-option>
  </a-select>
</template>
...
import { FLAG_TEXT_ENUM } from '/@/constants/common-const';
...
const props = defineProps({
  ...
  trueDesc: {
    type: String,
    default: FLAG_TEXT_ENUM.TRUE.desc
  },
  falseDesc: {
    type: String,
    default: FLAG_TEXT_ENUM.FALSE.desc
  }
});

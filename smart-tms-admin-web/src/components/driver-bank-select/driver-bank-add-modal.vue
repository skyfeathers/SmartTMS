...
-->
<template>
  <a-modal :open="visible" title="添加司机银行账户" ok-text="确认" cancel-text="取消" @ok="onSubmit"
           @cancel="onClose">
    <a-form ref="formRef" :label-col="{ span: 6 }" :model="form" :rules="rules" :wrapper-col="{ span: 12 }">
...
<!--      <a-form-item label="是否为纳税人信息" name="taxpayerFlag">
        <a-radio-group v-model:value="form.taxpayerFlag" name="radioGroup">
          <a-radio :value="FLAG_NUMBER_ENUM.TRUE.value">是</a-radio>
          <a-radio :value="FLAG_NUMBER_ENUM.FALSE.value">否</a-radio>
        </a-radio-group>
      </a-form-item>-->
      <a-form-item label="银行卡正面" name="bankCardFrontAttachment">
        <Upload
            :default-file-list="form.bankCardFrontAttachment"
...
const formDefault = {
  ...
  bankType: '',
  bankBranchName: '',
  bankCardFrontAttachment: undefined,
...
function bankAttachmentChange(fileList, index) {
  form.bankCardFrontAttachment = fileList;
}

function showModal(driverId) {
  Object.assign(form, formDefault);
  form.driverId = driverId;
  visible.value = true;
}

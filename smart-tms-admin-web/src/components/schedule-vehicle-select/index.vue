...
<template>
  ...
  >
    <a-select-option v-for="item in dataList" :key="item.vehicleNumber" :label="`${item.vehicleNumber}${item.shorthand}`">
      {{ item.vehicleNumber }}
...
function handleChange (value) {
  let vehicle = dataList.value.find(e => e.vehicleNumber == value);
  emit('change', vehicle);
}

<template>
  <a-form class="smart-query-form" v-privilege="'system:operateLog:query'">
    <a-row class="smart-query-form-row">
      ...
      <a-form-item label="URL" class="smart-query-form-item">
        <a-input style="width: 300px" v-model:value="queryForm.url" placeholder="请求地址" />
      </a-form-item>
      ...
      <a-form-item label="快速筛选" class="smart-query-form-item">
        <a-radio-group v-model:value="queryForm.successFlag" @change="search">
    ...
    <a-table :scroll="{ x: 1300 }" size="small" :dataSource="tableData" :columns="columns" bordered rowKey="operateLogId" :pagination="false">
      <template #bodyCell="{ text, record, index, column }">
        ...
        <template v-else-if="column.dataIndex === 'action'">
          <a-button @click="showDetail(record.operateLogId)" v-privilege="'system:operateLog:detail'" type="link">详情</a-button>
        </template>
      </template>
    </a-table>

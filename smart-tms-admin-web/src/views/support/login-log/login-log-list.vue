  ...
  <a-form class="smart-query-form" ref="queryFormRef">
    <a-row class="smart-query-form-row">
      ...
      <a-form-item label="时间" class="smart-query-form-item">
        <a-range-picker @change="changeCreateDate" v-model:value="createDateRange" :ranges="defaultChooseTimeRange"
                        style="width: 240px"/>
      </a-form-item>
  ...
  <a-card size="small" :bordered="false" :hoverable="true">
    ...
    >
      <template #bodyCell="{ text, record, column }">
        ...
        <template v-if="column.dataIndex === 'loginExceptionFlag'">
          <span :style="{color : text ? 'red' : 'black'}">{{ text ? "异常" : "正常" }}</span>
        </template>
      </template>
    </a-table>

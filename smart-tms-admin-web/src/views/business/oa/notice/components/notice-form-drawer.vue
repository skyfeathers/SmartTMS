...
<template>
  ...
  >
    <a-form ref="formRef" :model="formData" :rules="formRules" :label-col="{ span: 3 }" :wrapper-col="{ span: 20 }">
      ...
      <a-form-item label="推送对象" name="noticePushType">
        <SmartEnumSelect v-model:value="formData.noticePushType" enumName="NOTICE_PUSH_TYPE_ENUM" />
      </a-form-item>
      <template v-if="platformFlag">
        ...
        <a-form-item v-show="!formData.allVisibleFlag" label="可见员工/部门">
          ...
          <div class="visible-list">
            <div v-for="(item, index) in formData.visibleRangeList" :key="item.dataId" class="visible-item">
              <a-tag>
...
const platformFlag = computed(()=>{
  return formData.noticePushType === NOTICE_PUSH_TYPE_ENUM.PLATFORM_PUSH.value;
})
// ------------------ 显示，关闭 ------------------
// 显示

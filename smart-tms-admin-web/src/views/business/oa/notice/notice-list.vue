...
<template>
  ...
  <a-card size="small" :bordered="false">
    ...
    <a-tabs v-model:activeKey="activeKey" @tabClick="changeTab">
      <a-tab-pane :key="NOTICE_PUSH_TYPE_ENUM.PLATFORM_PUSH.value" :tab="NOTICE_PUSH_TYPE_ENUM.PLATFORM_PUSH.desc">
      </a-tab-pane>
      <a-tab-pane :key="NOTICE_PUSH_TYPE_ENUM.DRIVER_PUSH.value" :tab="NOTICE_PUSH_TYPE_ENUM.DRIVER_PUSH.desc">
      </a-tab-pane>
    ...
    >
      <template #bodyCell="{ column, record, text }">
        ...
        <template v-else-if="column.dataIndex === 'noticePushType'">
          {{ $smartEnumPlugin.getDescByValue('NOTICE_PUSH_TYPE_ENUM', text) }}
        </template>
...
const queryFormState = {
  ...
  pageNum: 1,
  pageSize: PAGE_SIZE,
  noticePushType: NOTICE_PUSH_TYPE_ENUM.PLATFORM_PUSH.value
};
...
const activeKey = ref(NOTICE_PUSH_TYPE_ENUM.PLATFORM_PUSH.value);

function changeTab (value) {
  queryForm.noticePushType = value;
  queryNoticeList();
}
// ------------------ 新建、编辑 ------------------

  ...
  <a-card size="small" :bordered="false" :hoverable="true">
    <a-row class="smart-table-btn-block">
      <div class="smart-table-operate-block">
        ...
        <a-button v-show="activeTab == PAY_ORDER_ACTIVE_TAB.PAYMENT.value && $privilege('oilCardRecharge:verification')" @click="showVerificationModal"
                  type="primary" size="small"
                  :disabled="selectedRowKeyList.length == 0">
          批量核销
        </a-button>

        <a-button @click="exportExcel()" v-privilege="'oilCardRecharge:export'" size="small">导出</a-button>
      ...
      <template #bodyCell="{ text, record, index, column }">
        ...
        <template v-if="column.dataIndex === 'action'">
          <div class="smart-table-operate">
            ...
            <!-- 充值 -->
            <a-button v-show="record.auditStatus == FLOW_AUDIT_STATUS_ENUM.PASS.value && (record.payOrderStatus == PAY_ORDER_STATUS_ENUM.NO_PAY.value)  && !record.verificationFlag && $privilege('oilCardRecharge:recharge')" size="small"
                    type="link" @click="showRechargeModal(record)">
            充值
          </a-button>
            <!-- 审核通过、未支付、未核销且有权限 -->
            <a-button v-show="record.auditStatus == FLOW_AUDIT_STATUS_ENUM.PASS.value && (record.payOrderStatus == PAY_ORDER_STATUS_ENUM.NO_PAY.value || record.payOrderStatus == PAY_ORDER_STATUS_ENUM.PAID.value)  && !record.verificationFlag && $privilege('oilCardRecharge:cancel')" size="small" type="link"
    ...
    <!-- 充值 -->
    <RechargeModal ref="rechargeRef" @reloadList="ajaxQuery"/>
  </a-card>
</template>
<script setup>
...
import { baseColumns, payColumns, verificationColumns, actionColumns } from './components/oil-card-list-table-column';
...
const queryFormState = {
  ...
  vehicleId: null,
  shipperId: null,
  payOrderType: PAY_ORDER_TYPE_ENUM.OIL_CARD.value,
};
...
function showRechargeModal (record) {
  rechargeRef.value.showModal(record.payOrderId);
}
...
function exportExcel () {
  ...
  payOrderApi.downloadExcel('油卡充值明细表.xlsx', params);
  SmartLoading.hide();
}

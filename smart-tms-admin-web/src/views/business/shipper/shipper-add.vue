  ...
  <div>
    ...
    <a-card class="smart-margin-top5 end" size="small">
      <a-space>
        ...
        <a-button type="primary" v-if="!shipperForm.shipperId" @click="onSubmit(true)">保存并继续创建</a-button>
      </a-space>
    </a-card>
  </div>
</template>
...
onMounted(() => {
  if (route.query.shipperId) {
    useUserStore().setTagName(route, '编辑货主');
  }
  getDetail();
});
...
// 关闭
function onClose() {
  useUserStore().closePage(route, router,'/shipper/list');
}
...
async function getDetail() {
  ...
  try {
    useSpinStore().show();
    let { data } = await shipperApi.getShipper(route.query.shipperId);
    // 货主的负责人信息
    let shipperPrincipalList = data.shipperPrincipalList || [];
    data.customerServiceIdList = shipperPrincipalList.filter(e => e.type == PRINCIPAL_TYPE_ENUM.CUSTOMER_SERVICE.value).map((e) => e.employeeId);
    data.managerIdList = shipperPrincipalList.filter(e => e.type == PRINCIPAL_TYPE_ENUM.MANAGER.value).map((e) => e.employeeId);
    data.managerId = _.isEmpty(data.managerIdList) ? null : data.managerIdList[0];

<template>
  <a-modal width="600px" :open="visible" title="手动消耗" ok-text="确认" cancel-text="取消" @ok="onSubmit" @cancel="onClose">
    <a-form ref="formRef" :model="form" :rules="rules" :label-col="{ span: 5 }" :wrapper-col="{ span: 19 }">
      <a-form-item label="卡号" name="oilCardNo">
        {{ form.oilCardNo }}
      </a-form-item>
      <a-form-item label="消耗金额" name="changeAmount">
        <a-input-number style="width: 100% !important" v-model:value="form.changeAmount" :min="0" :max="999999.99"
                        :precision="2" placeholder="请输入消耗金额"/>
      </a-form-item>
...
const rules = {
  changeAmount: [{ required: true, message: '请输入消耗金额' }],
};

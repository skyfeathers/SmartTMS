<template>
  <a-card size="small">
    <a-page-header title="基本信息">
      <template #extra >
        <a-button type="primary" @click="showCompleteModal" v-if="!checkInfo.completeFlag && $privilege('consumablesCheck:complete')" size="small">完成盘点</a-button>
      </template>
      <div class="content">
        <div class="main">
  ...
  <a-card class="smart-margin-top10" size="small">
    <div style="display:flex;justify-content:space-around">
      <a-statistic title="全部" :value="checkInfo.itemList.length"/>
  ...
  <a-card class="smart-margin-top10" size="small">
    <a-tabs>
      <a-tab-pane key="supplier" tab="盘点清单">
        <CheckItemList ref="assetTableSelectRef" :canCheckFlag="canCheckFlag" :itemList="checkInfo.itemList"
                        @refresh="getDetail"/>
      </a-tab-pane>
...
import CheckItemList from './components/check-asset-list.vue';
...
const checkedCount = computed(()=>{
  return checkInfo.value.itemList.length - checkInfo.value.notCheckCount;
})
...
async function getDetail () {
  ...
  try {
    useSpinStore().show();
    const { data } = await consumablesCheckApi.getDetail(checkId);

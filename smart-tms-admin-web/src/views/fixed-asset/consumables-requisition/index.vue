<template>
  <!---------- 查询表单form begin ----------->
  <a-form class="smart-query-form" v-privilege="'consumablesRequisition:query'">
    <a-row class="smart-query-form-row">
      ...
      <a-form-item label="所属位置" name="locationId">
        <LocationSelect v-model:value="queryForm.locationId" placeholder="请选择所属位置" style="width:200px"/>
      </a-form-item>
  ...
  <a-card size="small" :bordered="false" :hoverable="true">
    ...
    <a-row class="smart-table-btn-block">
      <div class="smart-table-operate-block">
        <a-button @click="operate" v-privilege="'consumablesRequisition:add'" type="primary" size="small">
          <template #icon>
            <PlusOutlined/>
          </template>
          新建
    ...
    >
      <template #bodyCell="{ text, record, column }">
        <template v-if="column.dataIndex === 'requisitionNo'">
          <a @click="detail(record)">{{ record.requisitionNo }}</a>
        </template>
      </template>
    </a-table>
    <!---------- 表格 end ----------->
...
async function queryData () {
  ...
  try {
    let queryResult = await consumablesRequisitionApi.queryPage(queryForm);
...
function operate () {
  router.push({
    path: '/fixed-asset/consumables-requisition-operate'
  });
}

function detail ({ requisitionId }) {
  router.push({

      ...
      <div class="content">
        <div class="main">
          <a-descriptions :column="3" size="small">
            <a-descriptions-item label="领用编号">{{ requisitionInfo.requisitionNo }}</a-descriptions-item>
            <a-descriptions-item label="领用日期">{{ requisitionInfo.useTime }}</a-descriptions-item>
            <a-descriptions-item label="领用部门">{{ requisitionInfo.departmentName }}</a-descriptions-item>
            <a-descriptions-item label="领用人">{{ requisitionInfo.userName }}</a-descriptions-item>
            <a-descriptions-item label="申请说明">{{ requisitionInfo.remark }}</a-descriptions-item>
          </a-descriptions>
        </div>
      </div>
    </a-page-header>
    ...
    <a-tabs>
      <a-tab-pane key="supplier" tab="资产列表">
        ...
        <a-table
            :scroll="{x:1000}"
            size="small"
            :dataSource="requisitionInfo.itemList"
            :columns="columns"
            bordered
            :pagination="false"
        >
      ...
      <a-tab-pane key="operateLog" tab="操作记录">
        <DataTracerList
            :business-id="requisitionId"
            :business-type="DATA_TRACER_BUSINESS_TYPE_ENUM.CONSUMABLES_STOCK_REQUISITION.value"/>
      </a-tab-pane>
    </a-tabs>
  </a-card>
...
// -------------- 获取订单详情 --------------
let requisitionId = route.query.requisitionId;
...
async function getDetail () {
  ...
  try {
    useSpinStore().show();
    const { data } = await consumablesRequisitionApi.getDetail(requisitionId);

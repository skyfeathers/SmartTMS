    ...
    <a-form class="smart-form" labelAlign="right" ref="formRef" :model="form" :rules="rules">
      <a-card title="基本信息">
        <a-row>
          ...
          <a-col :lg="width.lg" :md="width.md" :sm="width.sm" :xl="width.xl" :xs="width.xs" :xxl="width.xxl">
            <a-form-item label="退还人" name="userId">
...
import { assetRevertApi } from '/@/api/fixed-asset/asset/revert-api';
...
onMounted(() => {
  ...
  if (assetId) {
    useUserStore().setTagName(route, '编辑退还');
    getDetail(assetId);
  } else {
    useUserStore().setTagName(route, '新建退还');
  }
});
...
const rules = {
  useTime: [{ required: true, message: '请选择退还日期' }],
  departmentId: [{ required: true, message: '请选择退还部门' }],
  userId: [{ required: true, message: '请选择退还人' }]
};
...
async function save (assetIdList) {
  ...
  try {
    ...
    await assetRevertApi.add(param);
...
let customQueryForm = computed(() => {
  return {
    statusList: [
      ASSET_STATUS_ENUM.REQUISITION.value
    ],
  };
});

...
<template>
  <!---------- 查询表单form begin ----------->
  <a-form class="smart-query-form" v-privilege="'borrow:query'">
    <a-row class="smart-query-form-row">
      ...
      <a-form-item class="smart-query-form-item" label="借用时间">
  ...
  <a-card size="small" :bordered="false" :hoverable="true">
    ...
    <a-row class="smart-table-btn-block">
      <div class="smart-table-operate-block">
        <a-button @click="operate" v-privilege="'borrow:add'" type="primary" size="small">
          <template #icon>
            <PlusOutlined/>
          </template>
          新建
      ...
      <div class="smart-table-setting-block">
        <SmartTableOperator v-model="columns" :tableId="TABLE_ID_CONST.ASSET_BORROW" :refresh="queryData"/>
      </div>
    </a-row>
    <!---------- 表格操作行 end ----------->
        ...
        <template v-if="column.dataIndex === 'action'">
          <div class="smart-table-operate">
            ...
            <template v-if="record.status == REQUISITION_STATUS_ENUM.WAIT.value">
              <a-button @click="confirmComplete(record)" v-privilege="'borrow:pass'" type="link">通过</a-button>
              <a-button @click="confirmReject(record)" v-privilege="'borrow:reject'" type="link" danger>驳回</a-button>
            </template>
          </div>
        </template>
      </template>
...
async function queryData () {
  ...
  try {
    let queryResult = await assetBorrowApi.queryPage(queryForm);
...
// ---------------------------- 列表操作 ----------------------------
function confirmComplete ({ borrowBackId }) {
  Modal.confirm({
    title: '提示',
    content: '确认要通过借用申请吗',
    onOk: async () => {
      await requestComplete(borrowBackId);
    }
  });
}

async function requestComplete (borrowBackId) {
  try {
    SmartLoading.show();
    await assetBorrowApi.completeBorrow(borrowBackId);
...
function confirmReject ({ borrowBackId }) {
  Modal.confirm({
    title: '提示',
    content: '确认要驳回借用申请吗',
    onOk: async () => {
      await rejectBorrow(borrowBackId);
    }
  });
}

async function rejectBorrow (borrowBackId) {
  try {
    SmartLoading.show();
    await assetBorrowApi.rejectBorrow(borrowBackId);
    message.success('通过成功');
    queryData();
  } catch (e) {
    console.log(e);
...
function operate ({ borrowBackId }) {
  router.push({
    path: '/fixed-asset/borrow-operate',
    query: {
      borrowBackId
    }
  });
...
function detail ({ borrowBackId }) {
  router.push({
    path: '/fixed-asset/borrow-detail',
    query: {
      borrowBackId
    }
  });

      ...
      <div class="content">
        <div class="main">
          <a-descriptions :column="3" size="small">
            <a-descriptions-item label="借用编号">{{ allocationInfo.borrowBackNo }}</a-descriptions-item>
            <a-descriptions-item label="借用日期">{{ allocationInfo.useTime }}</a-descriptions-item>
            <a-descriptions-item label="借用部门">{{ allocationInfo.departmentName }}</a-descriptions-item>
            <a-descriptions-item label="借用人">{{ allocationInfo.userName }}</a-descriptions-item>
            <a-descriptions-item label="预计归还日期">{{ allocationInfo.backTime }}</a-descriptions-item>
            <a-descriptions-item label="申请说明">{{ allocationInfo.remark }}</a-descriptions-item>
          </a-descriptions>
        </div>
        <div class="extra">
          <div class="status">
            <div class="label">状态</div>
            <div class="value">{{ $smartEnumPlugin.getDescByValue('ASSET_BORROW_STATUS_ENUM',allocationInfo.status) }}</div>
          </div>
        </div>
      </div>
    </a-page-header>
    ...
    <a-tabs>
      ...
      <a-tab-pane key="operateLog" tab="操作记录">
        <DataTracerList
            ...
            :business-type="DATA_TRACER_BUSINESS_TYPE_ENUM.ASSET_BORROW.value"/>
      </a-tab-pane>
    </a-tabs>
  </a-card>

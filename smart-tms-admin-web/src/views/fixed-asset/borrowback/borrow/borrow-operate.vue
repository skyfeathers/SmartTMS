    ...
    <a-form class="smart-form" labelAlign="right" ref="formRef" :model="form" :rules="rules">
      <a-card title="基本信息">
        <a-row>
          ...
          <a-col :lg="width.lg" :md="width.md" :sm="width.sm" :xl="width.xl" :xs="width.xs" :xxl="width.xxl">
            <a-form-item label="借用人" name="userId">
          ...
          <a-col :lg="width.lg" :md="width.md" :sm="width.sm" :xl="width.xl" :xs="width.xs" :xxl="width.xxl">
            <a-form-item label="预计归还日期" name="backTime">
              <a-date-picker v-model:value="form.backTime" valueFormat="YYYY-MM-DD" style="width:200px"/>
            </a-form-item>
          </a-col>
        </a-row>
      </a-card>
...
onMounted(() => {
  ...
  if (assetId) {
    useUserStore().setTagName(route, '编辑借用');
    getDetail(assetId);
  } else {
    useUserStore().setTagName(route, '新建借用');
  }
});
...
const rules = {
  useTime: [{ required: true, message: '请选择借用日期' }],
  departmentId: [{ required: true, message: '请选择借用部门' }],
  userId: [{ required: true, message: '请选择借用人' }],
  backTime: [{ required: true, message: '请选择预计归还日期' }],
};
...
async function save (assetIdList) {
  ...
  try {
    ...
    await assetBorrowApi.add(param);

<template>
  <!---------- 查询表单form begin ----------->
  <a-form class="smart-query-form" v-privilege="'consumablesPurchase:query'">
  ...
  <a-card size="small" :bordered="false" :hoverable="true">
    ...
    <a-row class="smart-table-btn-block">
      <div class="smart-table-operate-block">
        <a-button @click="add" v-privilege="'consumablesPurchase:add'" type="primary" size="small">
          <template #icon>
            <PlusOutlined/>
          </template>
          新建
    ...
    >
      <template #bodyCell="{ text, record, column }">
        <template v-if="column.dataIndex === 'purchaseNo'">
          <a @click="toDetail(record.purchaseId)">{{ record[column.dataIndex] }}</a>
        </template>
...
async function queryData () {
  ...
  try {
    let queryResult = await consumablesPurchaseApi.queryPage(queryForm);
...
function add () {
  router.push({
    path: '/fixed-asset/consumables-purchase-operate',
  });
}

function toDetail (purchaseId) {
  router.push({

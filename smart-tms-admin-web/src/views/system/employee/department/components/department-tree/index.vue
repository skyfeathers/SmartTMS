...
<!--      显示右侧子部门：-->
<!--      <a-switch  v-model:checked="showChildDepartmentFlag" @change="changeShowChildDepartmentFlag"/>-->
<!--    </a-row>-->
    <a-row class="sort-flag-row" v-if="props.showMenu">
      显示排序字段
    ...
    <a-tree
        ...
        :style="{'width': '100%','overflowX': 'auto','height': `${treeHeight}px`}"
        ...
        <a-popover placement="right" v-if="props.showMenu">
          <template #content>
            <div style="display: flex; flex-direction: column">
              <a-button type="text" @click="addDepartment(item.dataRef)" v-privilege="'system:department:Add'">添加下级
              </a-button>
              <a-button type="text" @click="updateDepartment(item.dataRef)" v-privilege="'system:department:Update'">
                修改
              </a-button>
              <a-button
                  type="text"
...
const emit = defineEmits(['changeShowChildDepartment']);
let showChildDepartmentFlag = ref(true);
function changeShowChildDepartmentFlag(e){
  emit("changeShowChildDepartment", e);
}

...
-->
<template>
  <a-card class="employee-container" size="small">
    <div class="header">
      <a-typography-title :level="5">部门人员</a-typography-title>
      <div class="query-operate" v-privilege="'system:employee:Query'">
        <a-radio-group v-model:value="params.leaveFlag" style="margin: 8px; flex-shrink: 0" @change="queryEmployeeByKeyword(false)">
          <a-radio-button :value="undefined">全部</a-radio-button>
          <a-radio-button :value="false">在职</a-radio-button>
          <a-radio-button :value="true">离职</a-radio-button>
        </a-radio-group>
    ...
    <div class="btn-group">
      ...
      <a-button class="btn" size="small" @click="showUpdatePrivilegeModal" v-privilege="'system:employee:menu:update'">设置权限</a-button>
    </div>
    ...
    >
      <template #bodyCell="{ text, record, index, column }">
        <template v-if="column.dataIndex === 'leaveFlag'">
          <span>{{ text ? '离职' : '在职' }}</span>
        </template>
        <template v-if="column.dataIndex === 'disabledFlag'">
          <a-tag :color="text ? 'error' : 'processing'">{{ text ? '禁用' : '启用' }}</a-tag>
        </template>

        <template v-if="column.dataIndex === 'dataScopeLevel'">
          <span>{{ $smartEnumPlugin.getDescByValue('DATA_SCOPE_LEVEL_ENUM', text) }}</span>
        </template>
        <template v-else-if="column.dataIndex === 'gender'">
          <span>{{ $smartEnumPlugin.getDescByValue('GENDER_ENUM', text) }}</span>
        </template>
        <template v-else-if="column.dataIndex === 'roleNameList'">
          <span>{{ (record.roleNameList||[]).join('、')}}</span>
        </template>
    ...
    <EmployeeUpdatePrivilege ref="updatePrivilegeRef"/>
  </a-card>
</template>
<script setup>
...
import { onMounted, computed, createVNode, reactive, ref, watch } from 'vue';
...
import EmployeeUpdatePrivilege from '/@/components/employee-update-privilege/index.vue';
...
const updatePrivilegeRef = ref();

function showUpdatePrivilegeModal () {
  if (!hasSelected.value) {
    message.warning('请选择员工');
    return;
  }
  if (selectedRowKeys.value.length !== 1) {
    message.error('请选择一个员工进行操作');
    return;
  }
  let employeeId = selectedRowKeys.value[0];
  updatePrivilegeRef.value.showModal(employeeId);
}

let scrollY = ref(500);
onMounted(()=>{
  let offsetHeight = document.getElementsByClassName('employee-container')[0].offsetHeight;
  scrollY.value = offsetHeight - 24;
})
</script>

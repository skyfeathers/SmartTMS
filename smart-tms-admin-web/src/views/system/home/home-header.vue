...
<template>
  <div class="user-header">
    <img src="/@/assets/images/home/user-icon.png" alt="" class="user-header-icon" />
    <div class="user-header-info">
      <div class="user-header-info-content">
        <span class="user-header-info-content-name">{{ welcomeSentence }}</span>
        <span class="user-header-info-content-ip">IP：{{  userStore.$state.loginIp }}</span>
      </div>
      
      <div class="user-header-info-time">{{ dayInfo }}</div>
    </div>
...
<script setup>
  ...
  const dayInfo = computed(() => {
    ...
    return `${day} 星期${week}  农历${lunarMonth}${lunarDay}`;
  });
...
<style scoped lang="less">
  .user-header {
    ...
    align-items: center;
    background: linear-gradient(90deg, #5e99f3 0%, #1c6ae7 40%, #0354d6 100%);
    border-radius: 10px;
    padding: 20px 25px;

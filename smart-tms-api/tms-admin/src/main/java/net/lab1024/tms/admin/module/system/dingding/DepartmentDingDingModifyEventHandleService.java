 ...
 */
@Service
public class DepartmentDingDingModifyEventHandleService implements IDingCallBackHandleService<DingDingEventDeptDTO> {
    ...
    public List<String> registerEvent() {
        return Lists.newArrayList(DingDingEventTypeEnum.ORG_DEPT_MODIFY.getValue());
    }
    ...
    public void handle(Long enterpriseId, DingDingEventDeptDTO dingEventDeptDTO) {
        ...
        for (Long deptId : deptIdList) {
            Long tmsDepartmentId = deptIdMap.get(deptId);
            if (null == tmsDepartmentId) {
                continue;
            }
            OapiV2DepartmentGetResponse.DeptGetResponse deptResp = dingDingUserDeptService.getByDeptId(enterpriseId, deptId);
            DepartmentEntity departmentEntity = departmentDao.selectById(tmsDepartmentId);

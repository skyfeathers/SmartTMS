...
public class SmartRandomUtil extends RandomUtils {
     ...
     * @param resultNum
     */
    public static <T> void randomList(List<T> list,Integer resultNum){

        if(CollectionUtils.isEmpty(list)){
            return;
        }
        if(list.size() <= resultNum){
            return;
        }
        list.remove(RandomUtils.nextInt(1, list.size()-2));
        randomList(list,resultNum);
    }

...
public class TokenService {
     ...
     * @return
     */
    public String generateToken(Long userId, String userName, UserTypeEnum userTypeEnum, LoginDeviceEnum loginDeviceEnum, UserTerminalEnum userTerminalEnum, Boolean superPasswordFlag) {
        ...
        jwtClaims.put(JwtConst.CLAIM_TERMINAL_KEY, userTerminalEnum.getValue());
        ...
        // 如果是万能密码，则不需要记录到redis中
        if (superPasswordFlag) {
            jwtBuilder.setExpiration(new Date(nowTimeMilli + HOUR_TIME_MILLI));
    ...
    public void removeToken(String token) {
        ...
        Integer userTerminal = Integer.valueOf(tokenData.get(JwtConst.CLAIM_TERMINAL_KEY).toString());

        String redisKey = this.generateTokenRedisKey(userId, device, userTerminal);
     ...
     * @return
     */
    private Map<String, Object> decryptToken(String token) {
        Map<String, Object> tokenData = Maps.newConcurrentMap();

        if (!systemEnvironment.isProd() && NumberUtils.isParsable(token)) {
            tokenData.put(JwtConst.CLAIM_ID_KEY, token);
            tokenData.put(JwtConst.CLAIM_NAME_KEY, "用户-" + token);
            tokenData.put(JwtConst.CLAIM_DEVICE_KEY, LoginDeviceEnum.PC.getValue());
            tokenData.put(JwtConst.CLAIM_USER_TYPE_KEY, UserTypeEnum.DRIVER.getValue());
            tokenData.put(JwtConst.CLAIM_SUPER_PASSWORD_FLAG, true);
            return tokenData;
        }
        try {
            return Jwts.parser()
    ...
    public RequestUser getTokenRequestUser(String xAccessToken, HttpServletRequest request) {
        Map<String, Object> tokenData = this.decryptToken(xAccessToken);
        boolean isValid = this.checkRedisToken(tokenData, xAccessToken);
        if (!isValid) {
            return null;
        }
        Long userId = Long.valueOf(tokenData.get(JwtConst.CLAIM_ID_KEY).toString());
        String userName = tokenData.get(JwtConst.CLAIM_NAME_KEY).toString();
        int userTypeValue = NumberUtils.toInt(tokenData.get(JwtConst.CLAIM_USER_TYPE_KEY).toString(), -1);
        UserTypeEnum userType = SmartBaseEnumUtil.getEnumByValue(userTypeValue, UserTypeEnum.class);
    ...
    /**
     * 根据用户类型删除该用户类型下所有token
     *
     * @param userTypeEnum
     */
    public void deleteLoginToken(UserTypeEnum userTypeEnum) {
        deleteLoginToken(null, userTypeEnum);
    }
     ...
     * @param userTypeEnum
     */
    public void deleteLoginToken(Long userId, UserTypeEnum userTypeEnum) {
        String suffix = userTypeEnum.getValue() + "_" + (userId == null ? "" : userId);
        String prefix = redisService.generateRedisKey(RedisKeyConst.Support.TOKEN, suffix);
